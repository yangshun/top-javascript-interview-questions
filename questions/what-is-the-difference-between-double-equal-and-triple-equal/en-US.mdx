...
[] == false → true Why? [] (empty array) is truthy, but when compared with false, it gets coerced into a primitive value. JavaScript converts false to a number (0). Then, it converts [] to a string ('') → and an empty string is also 0 when converted to a number. So, the comparison becomes:
Number([]) == Number(false);
0 == 0; // true
