...
In this example, the `greetName` function takes another function `greet` as an argument and executes it with the name `Alice`. The `greetName` function is a higher-order function because it is passed as an argument to another function.
